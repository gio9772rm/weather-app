"""Meteo Pro V5 — complete scientific dashboard, mounted at /pro/."""
...
PRIMARY_CFG = Settings.from_env()
CFG = PRIMARY_CFG
from v5_data import scoped_forecast, station_settings

try:
    CFG = station_settings(
        str(st.query_params.get("station", PRIMARY_CFG.station_id)), PRIMARY_CFG
    )
except ValueError:
    CFG = PRIMARY_CFG
...
@st.cache_data(ttl=600, show_spinner=False)
def forecast_data(station_id: str) -> pd.DataFrame:
    return scoped_forecast(station_id)


@st.cache_data(ttl=600, show_spinner=False)
def forecast_history_data(station_id: str) -> pd.DataFrame:
    return scoped_forecast(station_id, history=True)
...
@st.cache_data(ttl=600, show_spinner=False)
def health_data(station_id: str) -> dict[str, Any]:
    from v5_data import scoped_health

    return scoped_health(station_id)
...
def light_pollution_data(
    latitude: float, longitude: float
) -> tuple[LightPollutionEstimate | None, str | None]:
    try:
        return fetch_light_pollution(latitude, longitude), None
...
def render_station_comparison(
    ...
    selected = selected[selected["local_date"] >= period_start]
...
with st.sidebar:
    st.markdown("### 🌦️ Meteo Pro · V5")
    st.link_button("← Nuova home", "/", width="stretch")
    ...
    if app_section == "Meteo città":
        ...
        selected_global_station = st.selectbox(
            "Località · tutte le sezioni",
            identifiers,
            index=identifiers.index(active_station_id),
            format_func=lambda value: str(profile_lookup.loc[value, "display_name"]),
            key="global_station_selector",
        )
        if selected_global_station != active_station_id:
            st.query_params["station"] = selected_global_station
            st.rerun()
        ...
        if len(identifiers) > 1:
            st.caption(
                ...
                "nella scheda Stazione. Ogni località usa dati e previsioni separati."
            )
...
if app_section == "Meteo città":
    ...
    render_city_dashboard(
        ...
        dark_mode,
        local_forecast=forecast_data(PRIMARY_CFG.station_id),
    )
    st.stop()

with st.spinner("Carico i dati meteo aggiornati…"):
    station = station_data(max(observation_hours + 24, 240), active_station_id)
    forecast = forecast_data(active_station_id)
    forecast_history = forecast_history_data(active_station_id)
    is_primary_view = active_station_id == PRIMARY_CFG.station_id
    ensemble_guidance = ensemble_guidance_data() if is_primary_view else pd.DataFrame()
    official_air_observed = observed_air_data() if is_primary_view else pd.DataFrame()
    official_pollen_observed = (
        measured_pollen_data() if is_primary_view else pd.DataFrame()
    )
    climate_normals = climate_normals_data() if is_primary_view else pd.DataFrame()
    reference_climate_normals = reference_climate_data(active_station_id)
    official_alerts = official_alerts_data() if is_primary_view else pd.DataFrame()
...
with tab_forecast:
    ...
    if forecast.empty:
        ...
        if active_station_id != PRIMARY_CFG.station_id:
            st.info(
                "Questa località conserva previsioni e verifiche indipendenti. Le statistiche di Roma non vengono applicate qui."
            )
            st.link_button(
                "Verifica della località nella V5",
                f"/?page=forecast&station={active_station_id}",
            )
        else:
            scores = score_data()
...
with tab_astro:
    ...
    if astro.empty:
        ...
        from v5_astronomy import (
            PROFILES,
            observing_forecast,
            summarize_observing_nights,
        )

        observing_profile = st.selectbox(
            "Profilo osservativo",
            list(PROFILES),
            format_func=lambda key: PROFILES[key]["label"],
            index=2,
        )
        remaining_astro = observing_forecast(forecast, CFG, observing_profile)
        daily_astro = summarize_observing_nights(remaining_astro).head(7)
        ...
        if not daily_astro.empty:
            ...
            daily_table = pd.DataFrame(
                {
                    ...
                    "Qualità media residua": daily_astro["score"],
                    "Qualità migliore": daily_astro["weather_score_best"],
                    "Ore favorevoli residue (≥65)": daily_astro["remaining_good_hours"],
                    "Finestra continua h": daily_astro["continuous_hours"],
                    "Dati incompleti h": daily_astro["incomplete_hours"],
                    "Limite principale": daily_astro["limiting_factor"],
                    ...
                    "Stabilità proxy": _numeric_series(daily_astro, "stability_proxy"),
            ...
            st.caption(
                "Le ore favorevoli residue contano solo il futuro, dal crepuscolo "
                "all’alba del profilo scelto, con score ≥65 e dati completi. "
                "Le finestre non attraversano buchi nei dati. Medie e limiti si "
                "riferiscono allo stesso intervallo residuo; il dettaglio dei fattori "
                "è disponibile nella nuova pagina Cielo."
            )
        ...
        windows = pd.DataFrame(
            [
                {
                    "start": window["start"].tz_convert(CFG.local_timezone),
                    "end": window["end"].tz_convert(CFG.local_timezone),
                    "hours": window["hours"],
                    "score": summary["score"],
                    "clouds": summary["clouds_mean"],
                }
                for _, summary in daily_astro.iterrows()
        ...
        if windows.empty:
            ...
            for position, (_, window) in enumerate(windows.head(3).iterrows()):
                ...
                cols[position].metric(
                    ...
                    f"{window['hours']:.1f} h residue · nuvole notte {window['clouds']:.0f}%",
                )
