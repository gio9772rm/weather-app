...
def summarize_forecast_change(
    ...
    *,
    ...
    temp_delta = (
        (_number(merged, "temp_c_new") - _number(merged, "temp_c_old")).abs().mean()
    )
    rain_delta = (
        _number(merged, "rain_mm_new").fillna(0).sum()
        - _number(merged, "rain_mm_old").fillna(0).sum()
    )
    pop_delta = (
        _number(merged, "precip_probability_new").max()
        - _number(merged, "precip_probability_old").max()
    )
    gust_delta = (
        _number(merged, "wind_gust_kmh_new").max()
        - _number(merged, "wind_gust_kmh_old").max()
    )
    confidence_delta = (
        _number(merged, "confidence_new").mean()
        - _number(merged, "confidence_old").mean()
    )
    ...
    if abs(float(rain_delta or 0)) < 0.05:
        rain_text = "pioggia sostanzialmente invariata"
    else:
        direction = "in più" if float(rain_delta or 0) > 0 else "in meno"
        rain_text = f"{abs(float(rain_delta or 0)):.1f} mm di pioggia {direction}"
    detail = (
