    ...
    *,
    ...
    try:
        ...
        raise ObservedAirError(
            "aria osservata EEA: servizio non raggiungibile"
        ) from exc
    if not response.ok:
        raise ObservedAirError(
...
def _request_body(
    cfg: Settings, start: pd.Timestamp, end: pd.Timestamp
) -> dict[str, Any]:
    return {
...
def _latest_valid_measurements(
    ...
    for frame in frames:
        ...
        current["time"] = pd.to_datetime(
            current.get("Start"), utc=True, errors="coerce"
        )
...
def _metadata_map(content: bytes, wanted: set[str]) -> dict[str, dict[str, Any]]:
    ...
    try:
        ...
        member = next(
            name for name in archive.namelist() if name.endswith("DataExtract.csv")
        )
    ...
    *,
    ...
    for item in measurements.to_dict("records"):
        ...
        rows.append(
            {
                "source": "eea_utd_air",
                "station_id": details.get("station_id")
                or _station_code(sampling_point),
    ...
    selected = (
        candidates.sort_values(
            ["metric", "_distance_sort", "time"], ascending=[True, True, False]
        )
...
def archive_observed_air(frame: pd.DataFrame, engine: Engine | None = None) -> int:
    if frame.empty:
        return 0
    ensure_schema()
