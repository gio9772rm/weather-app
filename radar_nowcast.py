...
def _tile_position(
    ...
    world_y = (1.0 - math.asinh(math.tan(latitude_rad)) / math.pi) / 2.0 * scale
...
def _echo_fraction(
    content: bytes, pixel_x: int, pixel_y: int, radius: int = 10
) -> float:
    try:
...
def fetch_radar_nowcast(
    ...
    *,
    ...
    try:
        ...
        zoom = 8
        tile_x, tile_y, pixel_x, pixel_y = _tile_position(latitude, longitude, zoom)
        samples: list[tuple[pd.Timestamp, float]] = []
        for frame in frames:
            tile_url = f"{host}{frame['path']}/256/{zoom}/{tile_x}/{tile_y}/2/1_1.png"
            try:
