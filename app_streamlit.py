...
# --- helpers for plotting continuity ---
def _connect_lines_df(df, time_col, cols, limit=2):
    try:
        d = df.copy()
        ...
        d[cols] = d[cols].interpolate(method="time", limit=limit, limit_direction="both")
        d = d.reset_index()
        return d
    except Exception:
        return df
...
with tab1:
    ...
    if df_station.empty:
        ...
        if "Temperatura" in charts:
            ...
            data = _connect_lines_df(data, "TimeLocal", ["Temp_C"], limit=2)
        ...
        if "Umidità" in charts:
            ...
            data = _connect_lines_df(data, "TimeLocal", ["Humidity"], limit=2)
        ...
        if "Pressione" in charts:
            ...
            data = _connect_lines_df(data, "TimeLocal", ["Pressure_hPa"], limit=2)
        ...
        if "Vento" in charts:
            ...
            data = _connect_lines_df(data, "TimeLocal", [c for c in ["Wind_kmh","WindGust_kmh"] if c in data.columns], limit=2)
...
# --- Keep scroll position across reruns (prevents jump to top during Play) ---
def _keep_scroll():
